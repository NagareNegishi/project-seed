...
- **Workers** — subagents in `.claude/agents/`:
  ...
  - **Testers**: `blackbox-tester` (spec-derived, spawned at session start
    ...
    boolean logic).
  - **Advisers** (one axis each, spawned over the merged code): `correctness-adviser`,
    `security-adviser`, `design-adviser`, `simplicity-adviser`, `performance-adviser`,
    `docs-adviser`, `legal-adviser`, `change-discipline-adviser` (judges the diff
    against its mandate — see Guardrails). Each is its axis critic's review plus a
    scoped in-axis fix direction; the paired `*-critic` twins stay for
    interactive/human sessions, but the skill spawns the advisers.
...
2. For a unit carrying real design or security surface, spawn the pre-build gate
   (`security-adviser` + `design-adviser`) over the unit *spec* first. Surface its
...
5. Both suites pass → spawn the post-code review layer: **allocate** advisers from
   the eight-axis roster per unit (not all, always), plus `change-discipline-adviser`
...
Each adviser owns one axis; allocated per unit, not all-always (Session flow, step 5).

| Axis | Agent |
| --- | --- |
| Correctness (logic, edge cases, contract) | `correctness-adviser` |
| Security risk | `security-adviser` |
| Design / architecture | `design-adviser` |
| Redundancy, over-complication | `simplicity-adviser` |
| Performance, efficiency | `performance-adviser` |
| Documentation, comments | `docs-adviser` |
| Legal, licensing, compliance | `legal-adviser` |
| Change discipline (diff vs. its mandate) | `change-discipline-adviser` |
| Decision-coverage testing (optional) | `mcdc-tester` |
| Root-cause diagnosis on failure | `debugger` |

Advisers find problems in their lane with evidence per finding and carry a scoped
in-axis fix direction (route `fix`); a finding with no clear in-axis fix, or one
reaching another axis, routes to the user (`decide`), never straight to an
implementer. The `*-critic` twins report the same problems without a fix direction
and stay for interactive/human sessions.
...
- **Advisers** — read `route`: `fix` → triage `Problems` by severity into batched fix
  units, each carrying the adviser's in-axis `directions` (high/medium block close-out,
  low → build-log accepted risk); `decide` → surface the finding to the user (no clear
  in-axis fix, or it crosses an axis); `redrive` → stage the missing input and respawn,
  or record the uncovered axis (feeds the allocation `miss`); `accept` → record `Checked`.
...
bug. A post-hoc adviser catches this only after the wasted loops. Prevention sits
in the loop and the prompts. Two levers plus one backstop adviser.
...
**Backstop — `change-discipline-adviser`.** Judges the *diff against its mandate*,
which no quality adviser does. Checks: the change does only what the task asked (no
...
- **Allocation grade** — gitignored `build-orchestration/prompt-log/allocation.md`, per-unit
  (deferred, never live): which advisers were deployed vs skipped and why, judged for
...
- Roles for the id scheme: `impl`, `blackbox`, `whitebox`, `mcdc`, `adviser`,
...
`whitebox-tester`, `security-adviser`, `design-adviser`.
...
- **Report format — defer to each agent.** The skill does not re-impose one; each
  ...
  as final message); only the eight critics and their advisers carry the
  `Target · Problems · Checked · Out of scope` review form (advisers add a per-problem
  fix direction under `Problems`), testers and advisory deviate. Template + deviations:
